...
One for the upcoming 1.0, and another for the 0.2.x releases.

Most changes are backported to the latter, "public-facing" releases. During GSoC
we also improved on some aspects of our usage guidelines policies, such that
...
no un-deprecated constants left that fit a given criteria.

Such criteria was not completely well-defined from prior discussion, but it
generally included symbols often used in C to denote the "last" or "limit" value
in a list of other constants and/or enumeration variants.
...
I developed a small CLI utility that scanned the rust-lang/libc codebase, parsed
it into an in-memory representation consisting solely of constants, and exposed
a simple TUI to mark symbols as deprecated.

That allowed me to focus solely on banging my head against Internet resources to
find out the meaning behind some non-obvious constants used in upstream projects
for which we provide C bindings.

Upon finishing work on this, feedback from my mentor started a discussion around
potentially _not_ deprecating the symbols. I didn't quite participate in that
...
@musl-lfs-statement. Most non-LFS types these days are already 64-bits wide.
...
This really means there's no reason to keep bindings for the LFS functions. Once
we bind to them, users will be making calls to function pointers that lead to
the LFS functions and not to the non-LFS functions.
...
supported libc target in each supported libc implementation is arguably hard. A
combinatorial explosion of mental marshaling operations is an experience.
...
looking into other issues in the 1.0 release milestone. Granted, along the way I
had already opened some issues concerning potentially wrong bindings.

Most notably, work on an issue I had opened before even the bonding period to
solve some `time_t` concerns on Windows targets, got eventually solved. This was
mostly related to the default `time_t` bit width on Windows being wrong.
...
I also finished up some general module-wide clean-ups for whole target families,
though some of these remain unmerged and are still actively receiving feedback.
...
the way functions get called. This is still an open issue but progress is
actively being made on that front and the solution seems clear.
...
dynamically-linked library, the codegen in the Rust compiler won't jump to the
right symbol, but will only the perform the first jump in the equivalent MSVC
codegen.
...
Not because I was in charge of that progress.
...
I haven't included the bonding period project I worked on and used throughout
the first three weeks of GSoC, but that one can be found here
@github-bonding-period-project.

Following my mentor's suggestion, I plan on eventually refactoring that tool
...
We've had that feature renamed to `std` for a few years now, and it's either way
...
As it turns out, `ioctl` is meant to be a routine that takes a request constant
that is often (but not always) a signed 32-bit integer. POSIX dictates so, but
popular implementations like glibc make it unsigned.

The issue, though, lies in the fact some of these constants could be 64-bits
wide and not just 32-bits wide. This is actually quite a pain point because that
one argument neither has well-defined semantics.
...
We don't often merge some PR until it passes all CI jobs, so tests are almost
...
didn't blindly use them without expecting breakage in non-breaking releases.
...
This hasn't been merged yet because there's some nifty item resolution conflicts
...
FreeBSD targets, each with a different network interface.

My mentor recommended against this when they gave an initial review. I'm
...
I've had moderate success with the latter option. Ideally, we would just extend
our test harness as this is not the the first time we've found ourselves with
this usecase (though it has been during GSoC.)
...
This really means we now automatically add the private field to all such records
...
That I implemented recently and have had a PR open since in
@github-exhaustive-macro. After that, there's going to be need for annotating
all the types where we didn't already have private fields (for other reasons)
with the attribute.

This is so that we can backport those changes to the next stable release, while
...
guarantees. This means a new release could very well just drop support for older
API.
...
issue @github-safamilyt-issue in the 1.0 milestone. I've yet to receive a
response from the original author and to get further approval from maintainers
to move it forward.
...
Granted, the whole point of the transition I eventually settled on for one of my
GSoC goals was to move towards full 64-bit `time_t` and LFS types. This PR was
closed shortly after one of the maintainers made that clear to me.
...
This PR is currently on hold and will continue being so until we see whether the
1.0 release is really nearing. There's similar patchsets that are also on hold
for the same reason.
...
The CI job for this target had been disabled due to some segfaults caused by the
...
a `time_t` that defaults to having a 32-bit bit width. This was wrong, but we
...
value will attempt to write 64-bits worth of data instead of only 32. This is
unsound, as it touches on allocations with potentially differing provenance.
...
There's not much to comment here. This was possibly the PR that took the longest
...
I myself build a bunch of software from source through the Nix build system and
package manager. It takes a declarative approach to package management that
ensures you get reproducible builds across runs and (sometimes) machines.

In rust-lang/libc, though, we contributors seemed to be needing to set up manual
toolchain overrides to get scripts using `nightly` to pass. I thought this could
be improved by having a `rust-toolchain` file.

`rustup` (the Rust toolchain and component manager) can detect this file and
either switch or otherwise install the toolchain specified in that file. This is
automatic and ensures the user doesn't have to worry about overrides.

What I failed to realize was that this file is also considered to be a form of
commitment to downstream users on the toolchain required for the project to even
...
undergo multiple source control history clean ups, and (2) newlib is not tied to
any particular target but rather used by multiple targets.
...
slightly exotic contexts. Getting confirmation on all the changes took a while.

On my side of things, I realized as time went on that I had made a few mistakes
on the bindings, and so had to both go back and fix those, as well as report on
...
the initial patch only got modified in that respect. The plan here (and in other
similar PRs) is to avoid mass deprecation from forcing users into allowing
warnings.

The solution my mentor came up with was to instead have them all annotated with
a simple `FIXME` comment that would be used later on to find+replace those with
actual deprecations once we neared the 1.0 release.

Otherwise, having these deprecations scattered throughout stable 0.2.x releases
...
This got eventually refactored into a patch that used a recent patch by another
contributor adding support for the one build option that I used in my initial
patch. That PR got merged first, and addressed only a subset of uClibc targets.
...
not just marked for future deprecation.
...
target has some other symbol that casts integer literal `0` to this type.
...
explained the potential reason behind the failures. As it turns out, Windows has
some funny ways of handling calls to symbols defined in external DLLs.

By default (without enabling global optimization) in MSVC, a call to a symbol in
an external DLL will be lowered to essentially two instruction jumps. The first
one goes to the Import Adress Table of the executable.
...
runtime, the loader ensures those table slots get filled in with the function
...
value stored in the stub (which contains the address of the function pointer in
the DLL.)
...
That conflicts with our eventual goal of getting rid of all explicit linking in
rust-lang/libc.
...
upstream repo, I noticed there were some other issues in the way we handled some
of those bindings.
...
Admittedly, this would have taken far less work on the reviewers' side of things
...
The initial patch for this PR was extensively modified after a few reviews with
both my mentor and the target maintainer. The changes that affected symbol
availability were mostly reduced to none.

The initial patch included adding support for two build-time configuration
options that NuttX allows setting up in their implementation of libc.

The fact those were non-standard meant we decided against supporting them, as it
...
Apparently, we had been checking for a compile-time property in the host
...
As it happened with other patchsets, this ended up spiralling out into a full-on
clean up of the module where we keep these bindings, and that eventually got
split into three separate PRs.

The first few reviews from both my mentor and the target maintainer were made on
the original patch. As mentioned above, that one got split; Out of the three
current PRs, one has been merged.

The one that got merged only added some `FIXME` comments for LFS symbols such
that we can more easily mass-deprecate those once we near the 1.0 release.

The other ones, including this one, need further review from the target
...
Initially, I was against the change proposed by the contributor because I
believed we were simply meant to provide C bindings, without caring at all for
soundness.
...
const-qualification of the pointer.

In Rustland, fetching a raw const pointer to some allocation should under all
circumstances be avoided from being cast into a raw mutable pointer, lest that
allocation could be safely and mutably aliased at call site (e.g. through a
regular reference.)

But the signature doesn't encode those semantics, and it's unlikely that users
...
original PR, and I executed on it.

The changes were divided into two patches; One moved the `if_mib` bindings
(which already existed) into our `new` module (which keeps them all more aligned
with the same structure as used upstream.)
...
root-exported symbols will be removed.

The fact that our current support for testing submodules without reexporting
...
The history with this type and other `signal.h`-related types is that there are
a bunch of target-specific and macro-dependent definitions that we had to
replicate from upstream.

The difference with usptream is that the maintenability of our current module
organization is hard in these cases. As soon as some definition needs
special-casing, the definitions across all sibling nodes in the module tree now
also need special-casing.

That's why, on top of completing the definition of the type to reflect the right
fields from upstream libc implementations used on Linux, this patchset also
moves it to the `new` submodule.
...
After many reviews across most of the patchsets I submitted, I've been slowly
learning about the way Rust guarantees as much stability as possible across the
table.

Though I personally would still completely break my entire API if I think it's
best that I start from scratch, and I could not care less about downstream
consumers of my library.

This summer I also learnt that the BSDs align far better with my take on project
maintenance than does, say, Linux. They care not about breakage, and will do
what's best to keep the system lean and mean.

Still, I see the value in stability, and there's a reason why the 1.0 stability
commitment from foundational Rust crates is known across other language
ecosystems.

One thing that I can say I was impacted enough by that I changed my mind on was
the importance of discussion prior to implementation. I've always thought that
almost all talk is useless small talk.

I've also always thought that text is mostly filler needed for bureaucratic
reasons. I even overstepped my boundaries in a certain discussion on Zulip when
the MCP concerning OpenBSD targets was being discussed.

But thankfully my mentor had more patience with me than I probably would've had
in their place. Time and again, changes that I thought were ready or could be
immediately executed upon, were proven to require more careful thinking.

Hopefully, I'll keep getting better at this patience business.

This summer I also learnt about the _Compile Farm_ project and the AnyVM CI
service. The former allowed me to cross-reference headers from closed-source
platforms like AIX and to test on more exotic platforms like the GNU Hurd.

The latter is now another tool I can keep under my belt to consider for testing
on target OSs such as the GNU Hurd or any modern flavor of the BSDs.

I also got to properly set up SSH authentication as I had to use this to access
the remote machines that cfarm provided. That was overall quite fun to mess
with.

The things I learnt from researching the Windows function pointer issues were
also quite interesting. My knowledge thus far had been limited to the way ELF
does things, so this was my first exposition to both the COFF and PE formats.

Beyond that, I can only thank enough my org admin, Jakub Beránek, my mentor,
